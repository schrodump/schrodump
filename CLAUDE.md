...
- CI: `ci.yml` (dco, readme-sync, check, integration, executor images build, image build/smoke — the
  last of which stands the **composed stack** up and drives twenty-two steps through it: all four
  engines in both execution modes, verified by restore, three restored over live data, a postgres
  target verified over TLS against a CA it was handed, a catalog rebuild, a key rotation, a
  ...
  rather than letting a `docker/login-action` error surface a minute deep in buildx.
