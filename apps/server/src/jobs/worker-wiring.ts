          ...
          return await runner.withEphemeralService(
            ...
            async ({ host }): Promise<VerifyProof> => {
              ...
              await runRestorePipeline({
                ...
                console.error(`[REPRO2] ${engine} restore pipeline threw:`, e instanceof Error ? `${e.name}: ${e.message}` : e);
                throw e;
              }
              ...
              // eslint-disable-next-line no-console
              console.error(`[REPRO2] ${engine} assert exit=${assertRun.exitCode} count=${count} rawStdout=${JSON.stringify(Buffer.concat(chunks).toString("utf8"))} stderr=${JSON.stringify(assertRun.stderr)}`);
