...
| Shutdown cleanup is bounded by `SCHRODUMP_SHUTDOWN_GRACE_MS`, and some cases outlast it            | `SIGTERM` aborts the in-flight job: the executor container is killed, the scratch directory (holding the dump in clear) is released, and the job row is `FAILED` before the process exits — bounded by `SCHRODUMP_SHUTDOWN_GRACE_MS` (default 8s, under docker's 10s). The orchestrator's own stop grace has to exceed that by a few seconds; `compose.yaml` ships `stop_grace_period: 15s`, but a bare `docker stop` (10s) or a hand-written unit file is the operator's to configure. What still outlasts the grace — known cases, not a complete list: a `SIGKILL` that beats it; a Docker daemon that hangs during teardown; and a STAGED backup whose scratch release is waiting on an in-flight S3 multipart upload to finalize, which is not cancellable today (making it so is a follow-up, since it touches checksum and size accounting on the happy path). Those still rely on the boot-time scratch sweep and orphan recovery. |
