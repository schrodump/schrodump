...
| -------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| Restore execution runs for PostgreSQL only; MySQL/MariaDB and MongoDB restore are gated off        | Their `buildRestore` descriptors were not adapted to the staged-file executor nor smoke-tested. The server refuses them with a clear error and the UI disables the trigger; backup/verify for those engines is unaffected. Lift per engine once adapted and smoke-tested. |
