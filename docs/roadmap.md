...
| --------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| STAGED (directory) dumps are disabled: `parallelism > 1` degrades to a single stream          | A staged dump writes a DIRECTORY (`pg_dump -Fd`, mydumper) but the upload path reads the container's STDOUT, and nothing ever read that directory back — so a STAGED backup uploaded an EMPTY artifact (gzip+age headers, 318 bytes) while the dump tool exited 0 and the job read SUCCEEDED. Verified against a real 545 MB database. Because restore refuses STAGED artifacts and verify downgrades them to CHECKSUM, and a CHECKSUM verify passes on those 318 bytes, the artifact could even reach `VERIFIED`. `resolveExecutionMode` now degrades to STREAM with a warning rather than produce that: a real single-stream backup beats a fast lie. Re-enabling STAGED needs a directory pipeline on BOTH sides — tar-and-upload on the way out, untar-to-directory on the way back — which is the same missing piece the restore row below describes.                                                                                                                                                                                   |
