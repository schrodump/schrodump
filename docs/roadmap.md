...
| -------------------------------------------------------------------------------- | ------------------------------------------------------------------------------ |
| Restore execution runs for PostgreSQL only; MySQL/MariaDB and MongoDB restore are gated off        | Their `buildRestore` descriptors were not adapted to the staged-file executor nor smoke-tested. The server refuses them with a clear error and the UI disables the trigger. Verify's `FULL_RESTORE` level reuses the same restore pipeline: it runs for real (ephemeral sandbox, restore, table-count assertion) on PostgreSQL, and downgrades to `CHECKSUM` — never blocked — for the other three engines, so backup/verify stays fully functional there. Lift per engine once `buildRestore` is adapted and smoke-tested. |
