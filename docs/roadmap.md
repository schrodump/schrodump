...
| CI exercises one PostgreSQL, one MySQL/MariaDB and one MongoDB build per run, not the whole matrix | A MongoDB **replica set** is now stood up and probed by default, which closes the gap that mattered most: `isReplicaSet` is the single fact the entire oplog chain hangs from — it decides whether `buildDump` emits `--oplog`, whether that is recorded on the artifact, and whether a `FULL_CLUSTER` restore replays it — and until this it was unit-tested only. The engine images are overridable (`SCHRODUMP_TEST_POSTGRES_IMAGE`, `_MYSQL_IMAGE`, `_MONGO_IMAGE`), and CI runs a probe-only pass against MariaDB and the PostgreSQL range edges. A full backup-and-restore round trip through the **composed deployment** now runs on every pull request (`scripts/smoke-compose.sh`): compose up, the setup link, keys, a live destination canary, a real connection test, a backup, a `FULL_RESTORE` verify, a restore over the live database, a catalog rebuild from the bucket alone, and a key rotation with the pre-rotation artifact still readable. It exists because three defects shipped in the seam between the code and `compose.yaml` that no other test could see. What is still NOT covered there: STAGED mode, MySQL/MariaDB/MongoDB (verified by hand, no defect of their own), self-backup, and a replica-set round trip proving `--oplogReplay` end to end rather than the fact that feeds it.                                                                                                                                                                                                                                                                               |
