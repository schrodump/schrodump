...
| CI exercises one PostgreSQL, one MySQL/MariaDB and one MongoDB build per run, not the whole matrix | A MongoDB **replica set** is now stood up and probed by default, which closes the gap that mattered most: `isReplicaSet` is the single fact the entire oplog chain hangs from — it decides whether `buildDump` emits `--oplog`, whether that is recorded on the artifact, and whether a `FULL_CLUSTER` restore replays it — and until this it was unit-tested only. The engine images are overridable (`SCHRODUMP_TEST_POSTGRES_IMAGE`, `_MYSQL_IMAGE`, `_MONGO_IMAGE`), and CI runs a probe-only pass against MariaDB and the PostgreSQL range edges. A full round trip through the **composed deployment** now runs on every pull request (`scripts/smoke-compose.sh`), seventeen steps: compose up, the setup link, keys, a live destination canary, a real connection test, backups of PostgreSQL, MySQL, MariaDB and MongoDB in **both execution modes**, a `FULL_RESTORE` verify of each, a restore over live data for three of them, a catalog rebuild from the bucket alone, a key rotation with the pre-rotation artifact still readable, a self-backup sealed to escrow over the internal network, retention observed actually deleting without leaving part of a backup behind, and a signed notification arriving at a real listener. It exists because three defects shipped in the seam between the code and `compose.yaml` that no other test could see. What is still NOT covered there: a replica-set round trip proving `--oplogReplay` end to end. Trying to add one is what surfaced the limitation in the row below — it is not a coverage gap, it is a path that cannot be reached. |
| **A MongoDB replica set cannot be backed up at all** | `probeMongodb` reports `scope.databases` as *every* database the credential can list, and `worker-wiring` feeds `probe.scope` straight into `buildDump`, which refuses `isReplicaSet && scoped`. The target's own `scope` never reaches that decision — it only picks the connect database, which for MongoDB is always the `admin` authSource. So `scoped` is true for **every** credential: a broad one lists several databases, a narrow one lists its own. There is no configuration that reaches the `--oplog` branch, which means `sourceHasOplog` is never recorded and `--oplogReplay` never runs. The descriptor test that covers `--oplog` builds `isReplicaSet: true` with an **empty** scope — a combination the wiring cannot produce. This is not an oversight but two deliberate rules meeting: `MONGODB_SCOPE_TOO_BROAD` tells the operator to use a credential scoped to one database, and `MONGODB_OPLOG_REQUIRES_FULL_DUMP` requires no scope at all. Resolving it is a behaviour decision, not a patch — the likely answer is to derive the dump scope from the **target** rather than from probe discovery, an unscoped target meaning the full instance, but that changes what an unscoped MongoDB backup does on a standalone too, so it is left for an explicit call rather than taken quietly. |
