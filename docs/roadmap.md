...
| TABLE-scoped restore is withdrawn for every engine; a multi-database MySQL artifact restores whole | No adapter emitted a table-scoping flag — postgres emits `-n` for SCHEMA and nothing for TABLE, mysql/mariadb emit nothing at all — while the capability matrix advertised `TABLE` for all three and `runRestoreJob` validates against exactly that list. So a TABLE request ran `pg_restore --clean` (or the mysql client) over the WHOLE dump, dropping and replacing every table in the database to write one of them. Withdrawn, with the same return condition as mongo's: the flag, plus an integration test proving a neighbouring table survives. Separately and still live: mysql `DATABASE` restore is correct for a single-database artifact but NOT for one dumped with `--databases`, whose script carries `CREATE DATABASE`/`USE`/`DROP TABLE` for every database in it. Measured on mysql 8.4.10 — two databases dumped together, a row added to the second after the dump, the script restored "into" the first: the second went from two rows to one, the new row gone, client exit 0. That hazard is a property of the artifact rather than the engine, so it is written here instead of being encoded in a per-engine table. |
